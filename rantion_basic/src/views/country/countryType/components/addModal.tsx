...
const CheckFuncList = {
	...
	checkOutThreeWord: (rule: object, value: string, callback: Function) => {
...
const rules = {
	...
	threeWord: [{ validator: CheckFuncList.checkOutThreeWord }]
}
interface State {
	visible: boolean
	isShow: boolean
	searchForm: defs.basics.BaseCountryRequestParamsDto
}
interface Aform {
	validate: () => Promise<defs.basics.BaseCountryRequestParamsDto>
}
export default defineComponent({
	...
	methods: {
		...
		async save() {
			...
			const { success } = await API.basics.baseCountry.save.request(params)
			if (success) {
	...
	setup() {
		...
		function init(data?: defs.basics.BaseCountryRequestParamsDto, type?: string) {
			if (data) {
			...
			>
				...
				>
					...
					<a-form-item name="countryNameCn" label="中文名称">
						<a-input
							v-model={[this.searchForm['countryNameCn'], 'value']}
					...
					<a-form-item name="countryNameEn" label="英文名称">
						<a-input
							v-model={[this.searchForm['countryNameEn'], 'value']}
							disabled={this.isShow}
							placeholder="请输入英文名称"
						/>
					</a-form-item>
					<a-form-item name="digitalCode" label="数字码">
						<a-input
							v-model={[this.searchForm['digitalCode'], 'value']}
							disabled={this.isShow}
							placeholder="请输入数字码"
						/>
					</a-form-item>
					<a-form-item name="threeWord" label="三字码">
						<a-input
							v-model={[this.searchForm['threeWord'], 'value']}
							disabled={this.isShow}
							placeholder="请输入三字码"
						/>
					</a-form-item>
					<a-form-Item prop="continent" label="大洲">
						<form-config
							allowClear
							prop="continent"
							v-model={[this.searchForm['continent'], 'value']}
							placeholder="请选择大洲"
							disabled={this.isShow}
						/>
					</a-form-Item>
