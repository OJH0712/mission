...
interface ShowDetailRef {
	init: (data?: defs.basics.BaseCountryRequestParamsDto, type?: string) => void
}
export default defineComponent({
	...
	setup() {
		const { run, loading, data, getPagination, renderPagination, refresh } = useRequest(
			API.basics.baseCountry.page.request,
			{
				manual: true,
				pagination: true
			}
		)
		const { searchForm, expand, expandToggle } = useSearch<API.basics.baseCountry.page.Params>({})
		...
		async function enable(secondWord: string, status: number) {
			...
			const { success } = await API.basics.baseCountry.enable.request(params)
			if (success) {
		...
		function update(dto: defs.basics.BaseCountryResponseDto, type: string) {
		...
		async function deleteOne(No: string) {
			const params = { code: '' }
			params.code = No
			const { success } = await API.basics.baseCountry.deleteOne.request(params)
			if (success) {
				notification.success({
					message: '提示',
					description: '删除成功'
		...
		const columns: Rantion.TableColumns[] = [
			...
			},
			{
				...
				align: 'center',
				key: 'action',
				customRender: ({ record }: Rantion.RableSlotScope<defs.basics.BaseCountryResponseDto>) => (
					<a-space size="middle">
			...
			<div>
				<div class="top-search-from">
					<a-form class="ant-advanced-search-form" labelCol={{ span: 8 }} wrapperCol={{ span: 14 }}>
						<a-row>
							<a-col span={6}>
								<a-form-item name="countryNameCn" label="中文名称">
									<a-input v-model={[searchForm.value.countryNameCn, 'value']} placeholder="请输入中文名称" />
								</a-form-item>
							</a-col>
							<a-col span={6}>
								<a-form-item name="countryNameEn" label="英文名称">
									<a-input v-model={[searchForm.value.countryNameEn, 'value']} placeholder="请输入英文名称" />
								</a-form-item>
							</a-col>
							<a-col span={6}>
								<a-form-item name="digitalCode" label="数字码">
									<a-input v-model={[searchForm.value.digitalCode, 'value']} placeholder="请输入数字码" />
								</a-form-item>
							</a-col>
							{expand.value ? (
								<>
									...
									<a-col span={6}>
										<a-form-item name="threeWord" label="三字码">
											<a-input v-model={[searchForm.value.threeWord, 'value']} placeholder="请输入三字码" />
										</a-form-item>
									</a-col>
									<a-col span={6}>
										<a-form-Item prop="continent" label="大洲">
											<form-config
												allowClear
												prop="continent"
												v-model={[searchForm.value.continent, 'value']}
												placeholder="请选择大洲"
											/>
										</a-form-Item>
									</a-col>
