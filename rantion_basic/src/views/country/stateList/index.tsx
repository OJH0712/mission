...
interface ShowDetailRef {
	init: (data?: defs.basics.BaseStateRequestDto, type?: string) => void
}
export default defineComponent({
	...
	setup() {
		const { run, loading, data, getPagination, renderPagination, refresh } = useRequest(
			API.basics.baseState.page.request,
			{
				manual: true,
				pagination: true
			}
		)
		const { searchForm, expand, expandToggle } = useSearch<API.basics.baseState.page.Params>({})
		...
		async function enable(secondWord: string, status: number) {
			...
			const { success } = await API.basics.baseState.enable.request(params)
			if (success) {
		...
		function update(dto: defs.basics.BaseStateResponseDto, type: string) {
		...
		async function deleteOne(No: string) {
			const params = { code: '' }
			params.code = No
			const { success } = await API.basics.baseState.deleteOne.request(params)
			if (success) {
				notification.success({
					message: '提示',
					description: '删除成功'
		...
		const columns: Rantion.TableColumns[] = [
			...
			},
			{
				...
				align: 'center',
				key: 'action',
				customRender: ({ record }: Rantion.RableSlotScope<defs.basics.BaseStateResponseDto>) => (
					<a-space size="middle">
			...
			<div>
				<div class="top-search-from">
					<a-form class="ant-advanced-search-form" labelCol={{ span: 8 }} wrapperCol={{ span: 14 }}>
						<a-row>
							<a-col span={6}>
								<a-form-item name="stateNameCn" label="中文名称">
									<a-input v-model={[searchForm.value.stateNameCn, 'value']} placeholder="请输入中文名称" />
								</a-form-item>
							</a-col>
							<a-col span={6}>
								<a-form-item name="stateNameEn" label="英文名称">
									<a-input v-model={[searchForm.value.stateNameEn, 'value']} placeholder="请输入英文名称" />
								</a-form-item>
							</a-col>
							<a-col span={6}>
								<a-form-item name="stateCode" label="数字码">
									<a-input v-model={[searchForm.value.stateCode, 'value']} placeholder="请输入数字码" />
								</a-form-item>
							</a-col>
							{expand.value ? (
								<>
