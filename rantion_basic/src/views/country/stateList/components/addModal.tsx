...
interface State {
	visible: boolean
	isShow: boolean
	searchForm: defs.basics.BaseStateRequestParamsDto
}
interface Aform {
	validate: () => Promise<defs.basics.BaseStateRequestParamsDto>
}
export default defineComponent({
	...
	methods: {
		...
		async save() {
			...
			const { success } = await API.basics.baseState.save.request(params)
			if (success) {
	...
	setup() {
		...
		function init(data?: defs.basics.BaseStateRequestParamsDto, type?: string) {
			if (data) {
			...
			>
				...
				>
					...
					<a-form-item name="stateNameCn" label="中文名称">
						<a-input
							v-model={[this.searchForm['stateNameCn'], 'value']}
					...
					<a-form-item name="stateNameEn" label="英文名称">
						<a-input
							v-model={[this.searchForm['stateNameEn'], 'value']}
							disabled={this.isShow}
							placeholder="请输入英文名称"
						/>
					</a-form-item>
					<a-form-item name="stateCode" label="数字码">
						<a-input
							v-model={[this.searchForm['stateCode'], 'value']}
							disabled={this.isShow}
							placeholder="请输入数字码"
						/>
					</a-form-item>
